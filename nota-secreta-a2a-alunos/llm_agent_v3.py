...
class LLMAgent(BaseAgent):
    ...
    def _card_prompt_line(self, card: Dict[str, Any], idx: int) -> str:
        # v3: em vez de só 8 palavras-chave, mostra ao LLM um trecho real da
        # letra (já truncada em 80 palavras pelo Game Master). Hipótese: dar
        # conteúdo da música, e não um saco de palavras, melhora o casamento
        # dica<->carta e portanto os acertos.
        title = str(card.get("title", "")).strip()
        snippet = " ".join(str(card.get("lyrics", "")).split()[:24])
        return f"{idx}: título={title}; trecho=\"{snippet}\""
