...
"""Agente estratégico v2 (otimizado) para o jogo Nota Secreta.

Diferenças em relação ao llm_agent.py (baseline):
  1. _semantic_score reequilibrado: prioriza a sobreposição com a LETRA e a
     cobertura da dica, e rebaixa o título a desempate fraco. Motivo: uma boa
     dica evita de propósito as palavras do título, então o título quase nunca
     é o sinal correto e às vezes engana (decoy com palavra coincidente).
  2. send_clue com guarda de robustez + auto-consistência: se a dica do LLM
     degenerar (vazia, igual ao mock "memória tempo cidade") OU não apontar
     para a carta do narrador dentro da própria mão (dica vaga demais), cai
     para uma dica temática derivada da letra. Isso converte rodadas de
     narrador de 0 -> +3 e protege contra timeout/mock do serviço LLM.

Mantém EXATAMENTE a mesma interface A2A (5 tools) exigida pelo Game Master.
"""
...
class LLMAgent(BaseAgent):
    ...
    async def send_clue(self, lyrics: str, max_words: int = 6) -> Dict[str, Any]:
        ...
        # --- v2: guarda de robustez + auto-consistência -------------------
        # Se a dica degenerou (mock/timeout) ou está vaga demais (não aponta
        # para a carta do narrador nem dentro da própria mão), troca por uma
        # dica temática derivada da letra. Assim o narrador deixa de jogar a
        # rodada fora quando o LLM não coopera.
    ...
    def _semantic_score(self, card: Dict[str, Any], clue: str) -> float:
        """v2: letra é o sinal principal; título vira desempate fraco.

        Boas dicas evitam o título de propósito, então peso alto no título
        (como no baseline) majoritariamente engana. Aqui a letra e a cobertura
        da dica dominam, e usamos mais termos da letra (até 60) já que ela já
        vem truncada em 80 palavras pelo Game Master.
        """
        clue_terms = set(self._extract_keywords(clue))
        title_terms = set(self._extract_keywords(str(card.get("title", ""))))
        lyric_terms = set(self._extract_keywords(str(card.get("lyrics", "")))[:60])
        ...
        coverage = (lyric_overlap + 0.5 * title_overlap) / max(1, len(clue_terms))
        return (1.7 * lyric_overlap) + (1.6 * coverage) + (0.6 * title_overlap) + phrase_bonus
    ...
    def _is_degenerate_clue(self, clue: str) -> bool:
        """v2: detecta dica vazia/pobre ou a saída fixa do mock do LLM."""
        norm = self._normalize_text_for_match(clue)
        if not norm:
            return True
        if norm in _MOCK_SENTINELS:
            return True
        useful = [w for w in self._extract_keywords(clue)]
        return len(useful) < 2

    def _clue_points_to_card(self, clue: str) -> bool:
        """v2: a dica deve apontar para a carta do narrador dentro da própria mão.

        Proxy de "a dica não é vaga demais": se, entre as 4 cartas da mão, a
        carta escolhida não fica em 1º pela nossa pontuação semântica, a dica
        provavelmente não vai levar ninguém à carta certa -> narrador tira 0.
        """
    ...
    def _parse_loose_ranking(
        ...
        for raw in re.findall(r"\d+", response):
            idx = int(raw)
            # Aceita numeração 1-based: se vier um índice == n_options, mapeia
            # para o último (n_options-1). Demais índices válidos seguem 0-based.
            if idx == n_options:
                idx -= 1
...
def main() -> None:
    ...
    agent = LLMAgent(name=args.name or f"LLMAgentV2_{args.port}", llm_url=args.llm_url)
    app.register(agent)
